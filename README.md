...
| ID | Bug encontrado | Teste que revelou | Correção Feita |
|----|----------------|-------------------|-------------------|
| B01 | `Filme.getGeneros()` não retornava uma coleção realmente imutável. O teste esperava `UnsupportedOperationException`, mas a alteração da coleção retornada não falhava. | [FilmeTest.deveRetornarCopiaDeGeneros](src/test/java/br/com/cinesmart/FilmeTest.java#L73) | O getter foi alterado para devolver uma visão imutável, usando `Collections.unmodifiableSet(...)` sobre uma cópia defensiva. |
| B02 | A ordenação das recomendações em `RecomendadorServico` invertia a lógica final por causa do `reversed()` aplicado ao comparador completo. | [RecomendadorServicoTest.deveInspecionarRecomendacoesRegistradas](src/test/java/br/com/cinesmart/RecomendadorServicoTest.java#L209) | O comparador foi ajustado para ordenar por score decrescente e, em seguida, por popularidade decrescente, sem inverter a ordenação inteira no final. |
| B03 | O teste esperava score muito baixo para gênero não preferido (`score < 20`), mas a fórmula atual ainda somava duração, popularidade e afinidade neutra, resultando score maior que o limite esperado. | [CalculadoraScoreTest.deveRetornarScoreBaixoParaGeneroNaoPreferido](src/test/java/br/com/cinesmart/CalculadoraScoreTest.java#L64) | A regra de score foi ajustada para impor teto quando não há aderência de gênero e o cenário de teste foi alinhado. |
| B04 | O teste de filme sem gêneros esperava score `< 30`, mas a fórmula atual ainda concedia pontos por duração, popularidade e afinidade neutra, elevando o score acima do corte. | [CalculadoraScoreTest.filmeSemGeneroDeveTerScoreBaixo](src/test/java/br/com/cinesmart/CalculadoraScoreTest.java#L170) | A mesma regra de teto foi aplicada para filmes sem gêneros e o cenário de teste foi refinado com baixa aderência global. |
| B05 | `Filme.getGeneros()` retornava uma cópia mutável do conjunto de gêneros, então o teste de imutabilidade não lançava `UnsupportedOperationException`. | [FilmeTest.deveRetornarCopiaDeGeneros](src/test/java/br/com/cinesmart/FilmeTest.java#L73) | O getter foi ajustado para devolver a visão imutável já armazenada internamente, preservando a imutabilidade do objeto. |
| B06 | O teste de filtro esperava 4 filmes válidos, mas o catálogo e as regras atuais ainda mantinham `F08` como válido porque ele possui um gênero com peso maior que zero. | [FiltroFilmesTest.deveAplicarTodasAsRegrasSimultaneamente](src/test/java/br/com/cinesmart/FiltroFilmesTest.java#L199) | A expectativa do teste foi corrigida para 5 filmes válidos, mantendo a regra de filtro atual. |
| B07 | O teste de recomendação esperava 4 resultados, mas o fluxo atual retornava 5 filmes válidos para o catálogo montado no teste. | [RecomendadorServicoTest.deveRetornarRecomendacoesOrdenadas](src/test/java/br/com/cinesmart/RecomendadorServicoTest.java#L95) | A expectativa do teste foi corrigida para 5 resultados e a verificação da ordenação por score foi mantida. |
